...
class Sentinel2Downloader:
    ...
    def _setup(self, product_type, tiles, start_date, end_date, bands,
        ...
        bands = bands.intersection(BANDS)
        if not bands:
...
def split_image_and_save(image, crop_h, crop_w, save_crops=False,
                         saving_path=None):
    c, h, w = image.size()
    ...
    for i in range(n_h):
        for j in range(n_w):
            ...
            if save_crops:
                filename = saving_path + "_" + str(start_h) + "_" + str(end_h) + "_" + str(start_w) + "_" + str(end_w) + ".png"
                pil_image = to_pil_image(cropped)
